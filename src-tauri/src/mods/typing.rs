...
pub async fn get_word_meaning(
    ...
    let meaning: Option<String> = conn.query_row(
...
// 获取所有单词释义（返回 JSON 对象）
#[command]
pub async fn get_all_meanings(
    state: tauri::State<'_, DbState>,
) -> Result<String, String> {
    let conn = state.conn.lock().map_err(|e| format!("Failed to lock mutex: {}", e))?;
    
    let mut stmt = conn.prepare(
        "SELECT word, meaning FROM word_dictionary ORDER BY word"
    ).map_err(|e| format!("Failed to prepare statement: {}", e))?;
    ...
    if meanings.len() > 0 {
        println!("First few meanings: {:?}", meanings.iter().take(3).collect::<Vec<_>>());
    }
...
pub async fn batch_update_meanings(
    ...
    let meanings_map: serde_json::Map<String, serde_json::Value> = 
        serde_json::from_str(&meanings)
        .map_err(|e| format!("无效的 JSON 格式: {}", e))?;
    
    let now = chrono::Local::now().format("%Y-%m-%d %H:%M:%S").to_string();
    let mut conn = state.conn.lock().map_err(|e| format!("Failed to lock mutex: {}", e))?;
    
    let transaction = conn.transaction().map_err(|e| format!("Failed to start transaction: {}", e))?;
    ...
    transaction.commit().map_err(|e| format!("Failed to commit: {}", e))?;
