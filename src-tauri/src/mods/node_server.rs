...
#[tauri::command]
pub async fn stop_server(state: tauri::State<'_, ServerState>) -> Result<String, String> {
    let mut server_process = state.process.lock().unwrap();
...
#[tauri::command]
pub async fn get_server_status(state: tauri::State<'_, ServerState>) -> Result<bool, String> {
    let mut server_process = state.process.lock().unwrap();

    match server_process.as_mut() {
        Some(child) => Ok(child.try_wait().is_err()),
        None => Ok(false),
    }
}
