...
export default async function chatModule(fastify: FastifyInstance) {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    // === 发送私聊消息 ===
    socket.on("privateMessage", ({ target, message }: { target: string; message: string }) => {
      ...
      // 同时发送给自己（用于显示已发送的消息）
      socket.emit("privateMessage", { ...privateMsg, target: sender }); // 标识为自己发送
    ...
    socket.on("addFriend", (targetUsername: string) => {
      ...
      if (targetUsername === username) return socket.emit("error", "不能添加自己为好友");
      ...
      if (myPending.some(r => r.to === targetUsername && r.status === "pending")) {
    ...
    // === 处理好友请求（接受/拒绝） ===
    socket.on("handleFriendRequest", ({ from, accepted }: { from: string; accepted: boolean }) => {
      const username = onlineUsers.get(socket.id);
      if (!username) return;

      const targetRequests = pendingRequests.get(username) || [];
      const requestIdx = targetRequests.findIndex(r => r.from === from && r.status === "pending");
      if (requestIdx === -1) return socket.emit("error", "该请求不存在或已处理");
